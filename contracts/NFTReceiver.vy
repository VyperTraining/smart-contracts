...
def onERC721Received( _operator: address, _from: address, _tokenId: uint256, _data: Bytes[1024]=b"") -> bytes32:
    assert self.ownerOf[msg.sender][_tokenId] == _from

    return method_id("onERC721Received(address,address,uint256,bytes)", output_type=bytes32)

@nonpayable
@external
def setOwner(contract_address: address, _tokenId: uint256):
    nft_contract: ERC721 = ERC721(contract_address)
    owner: address = nft_contract.ownerOf(_tokenId)
    self.ownerOf[contract_address][_tokenId] = owner

@nonpayable
@external
def withdraw(contract_address: address, _tokenId: uint256):
